...
def _get_garmin_client() -> Garmin:
    ...
    if not garmin_email:
        raise ValueError(
            "GARMIN_EMAIL is required. Add it as a GitHub Actions secret."
        )

    if not garmin_password:
        raise ValueError(
            "GARMIN_PASSWORD is required. Add it as a GitHub Actions secret."
        )

    # The saved session directory is restored by the workflow before this runs.
    # garminconnect 0.3.6 first tries that saved session and refreshes it.
    # If Garmin rejects it, the library discards it and logs in fresh using
    # the email and password below. That is the self-healing behaviour.
